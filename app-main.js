...
startBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        if (faceDetectionEnabled) {
            setTimeout(() => {
                startFaceDetection().catch(e => {
                    console.warn('⚠️ Face detection failed, continuing without it:', e.message);
                });
            }, 1500);
        }
...
async function startVideoStream() {
    try {
        ...
        if (!currentSceneState || currentSceneState.length < 10) {
            currentSceneState = startPrompt;
            console.log('📍 Initial scene state set:', currentSceneState);
        }
...
async function startFaceDetection() {
    if (!window.faceApiLoaded || !faceDetectionEnabled) {
        console.log('⚠️ Face detection disabled or models not loaded');
        return;
    }
    ...
    try {
        ...
        const canvas = faceCanvas; // Use separate canvas for face detection
        ...
        console.log('✅ Video ready for face detection:', video.videoWidth, 'x', video.videoHeight);
        ...
        console.log('✅ Face canvas sized:', canvas.width, 'x', canvas.height);
        
        // Continuous face detection
        const detectFace = async () => {
            if (!faceDetectionEnabled || !isConnected) {
                return;
            }
            ...
            try {
                ...
                if (detections) {
                    ...
                    // Update if confidence is reasonable (lowered to 0.3)
                    if (maxScore > 0.3) {
                        ...
                        // Log emotion changes
                        if (oldEmotion !== maxEmotion) {
                            console.log(`😊 Emotion changed: ${oldEmotion} → ${maxEmotion} (${(maxScore * 100).toFixed(0)}%)`);
                        }
                        ...
                        // Also update sidebar info panel
                        if (emotionValueEl) {
                            emotionValueEl.textContent = `${emoji} ${maxEmotion}`;
                        }
                    }
                } else {
            ...
            // Continue detection
            if (faceDetectionEnabled && isConnected) {
                setTimeout(detectFace, 200); // Check every 200ms for smoother box
            }
        };
